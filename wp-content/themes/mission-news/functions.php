...
//----------------------------------------------------------------------------------
require_once( trailingslashit( get_template_directory() ) . 'woocommerce/functions.php' );
...
if ( ! function_exists( ( 'ct_mission_news_register_widget_areas' ) ) ) {
	function ct_mission_news_register_widget_areas() {
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Nota Left Sidebar', 'mission-news' ),
			'id'            => 'nota-left',
			'description'   => esc_html__( 'Widgets in this area will be shown left of the post content.', 'mission-news' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>'
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Nota Right Sidebar', 'mission-news' ),
			'id'            => 'nota-right',
			'description'   => esc_html__( 'Widgets in this area will be shown right of the post content.', 'mission-news' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>'
			...
			register_sidebar( array(
			'name'          => esc_html__( 'Buscador', 'mission-news' ),
			'id'            => 'buscador',
			'description'   => esc_html__( 'Widgets in this area will be shown right of the post content.', 'mission-news' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
	...
	function ct_mission_news_featured_image() {
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			...
			if ( is_singular() ) {
				...
				$featured_image = '<div class="bloque-imagen"><div class="featured-image">' . get_the_post_thumbnail( $post->ID, 'large' ) . '</div><figcaption>' .  wp_kses_post($caption) . '</figcaption><div style="float:none;clear:both;"></div></div>'; 
				...
				$featured_image = '<div class="featured-image"><a href="' . esc_url( get_permalink() ) . '">' . esc_html( get_the_title() ) . get_the_post_thumbnail( $post->ID, 'thumbnail' ) . '</a><figcaption>texto</figcaption></div>';
			}
		}
...
if ( ! function_exists( 'ct_mission_news_get_content_template' ) ) {
	function ct_mission_news_get_content_template() {
		...
		if ( is_home() || is_archive() || is_search()  || is_page(4919) ) {
...
if ( ! function_exists( ( 'ct_mission_news_post_byline' ) ) ) {
	function ct_mission_news_post_byline( $author, $date ) {
		...
		//print_r($post_author);
		//$post_author_label = get_the_author_meta( 'user_nicename', $post->post_author );
		$post_author_label = get_the_author_meta( 'user_nicename', $author );
		...
		//$post_date = date_i18n( get_option( 'date_format' ), strtotime( get_the_date('d.m.Y') ) );
		$post_date = date_i18n( get_option( 'date_format' ), strtotime( get_the_date('d.m.Y') ) );
		...
		if ( $author == 'no' ) {
			...
			echo '<a href="' . get_site_url() . '/author/' . $post_author_label . '" class="post-author">';
			printf( esc_html_x( '%s', 'This blog post was published by some author', 'mission-news' ), esc_html( $post_author ) );
			echo '</a>';
		} else {
			// translators: %1$s = the author who published the post. %2$s = the date it was published
			echo '<span class="post-date">';
			printf( esc_html_x( '%1$s', 'This blog post was published by some author on some date ', 'mission-news' ), esc_html( $post_date ) );
...
//----------------------------------------------------------------------------------
	...
	//if( isset($query->query_vars['post_type']) && $query->query_vars['post_type'] == 'event' ) {
		...
		if( isset($_GET['autor']) ) {
			...
			$query->set('meta_value', $_GET['autor']);
...
//----------------------------------------------------------------------------------
function ct_mission_news_output_last_updated_date() {
	
	global $post;

	if ( get_the_modified_date() != get_the_date() ) {
		$updated_post = get_post_meta( $post->ID, 'ct_mission_news_last_updated', true );
...
acf_add_local_field_group(array(
	'key' => 'group_5f235a8c4b1a3',
	'title' => 'Autor',
	'fields' => array(
		array(
			'key' => 'field_5f2363c45901f',
			'label' => 'Autor',
			'name' => 'autor',
			'type' => 'text',
			'instructions' => 'Ingrese el nombre del autor',
			'required' => 1,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
...
//----------------------------------------------------------------------------------
function ct_mission_news_register_elementor_locations( $elementor_theme_manager ) {
