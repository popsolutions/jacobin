...
$the_query = new WP_Query( array( 'posts_per_page' => 1, 'category__in' => array(1158) , 'category__not_in' => array(), 'order'   => 'DESC') ); 
...
<?php 
// The Query
$the_query = new WP_Query( array( 'posts_per_page' => 7, 'category__in' => array(1153) , 'category__not_in' => array(), 'order'   => 'DESC') );
// The Loop
...
if ( $the_query->have_posts() ) {
	...
	while ( $the_query->have_posts() ) {
		...
		$the_query->the_post();
		$content = get_the_content();
		echo '<article class="content-archive dos">';?>
		<?php ct_mission_news_featured_image(); ?>
		<div class="post-header">
		<h2 class='post-title'>
		<a href="<?php echo esc_url( get_permalink() ); ?>"><?php the_title(); ?></a>
...
<div>
<img src="https://jacobinlat.com/wp-content/uploads/2020/09/tapaNueva2.jpg">
...
// The Query
$the_query = new WP_Query( array( 'posts_per_page' => 1, 'category__in' => array(1153) , 'category__not_in' => array(), 'order'   => 'DESC') );
// The Loop
if ( $the_query->have_posts() ) {
...
<?php
 // The Query
$the_query = new WP_Query( array( 'posts_per_page' => 4, 'category__in' => array(1153) , 'category__not_in' => array(), 'order'   => 'DESC') );
// The Loop
if ( $the_query->have_posts() ) {
...
<?php
 // The Query
$the_query = new WP_Query( array( 'posts_per_page' => 8, 'category__in' => array(1153) , 'category__not_in' => array(), 'order'   => 'DESC') );
// The Loop
if ( $the_query->have_posts() ) {
