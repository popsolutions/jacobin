...
function mfnvb_admin_bar_menu( $admin_bar ){
	if( is_page() ){
	    ...
	    'title' => 'Edit with '. apply_filters('betheme_label', 'Muffin') .' Builder', // Label for this item
...
function mfnvb_ce_live_button($post) {
	if(in_array($post->post_type, array('page', 'post', 'portfolio', 'template', 'product'))){
		...
		if( get_post_status($post->ID) == 'publish' ){
			echo '<div style="float: right; margin: 15px 0;" class="mfn-live-edit-page-button"><a '. $replaced_logo .' href="' .admin_url('/post.php?post='. $post->ID .'&action='. apply_filters('betheme_slug', 'mfn') .''). '-live-builder" class="mfn-btn mfn-switch-live-editor button-hero mfn-btn-green button button-primary">Edit with '. apply_filters('betheme_label', "Muffin") .' Live Builder</a></div>';
		}else{
			echo '<div style="float: right; margin: 15px 0;" class="mfn-live-edit-page-button"><a '. $replaced_logo .' href="' .admin_url('/post.php?post='. $post->ID .'&preview=true&action='. apply_filters('betheme_slug', 'mfn') .'-live-builder'). '" class="mfn-btn mfn-switch-live-editor button-hero mfn-btn-green button button-primary">Edit with '. apply_filters('betheme_label', "Muffin") .' Live Builder</a></div>';
		}
	}
}
...
function mfnvb_list_row_actions( $actions, $post ) {
    if ( in_array($post->post_type, array("page", "post", "portfolio", "template", "product")) ) {
 		$actions[] = '<span class="mfn-edit-link"><a href="'.admin_url( 'post.php?post=' . $post->ID . '&action='. apply_filters('betheme_slug', 'mfn') .'-live-builder' ).'" aria-label="Edit with '. apply_filters('betheme_label', "Muffin") .' Live Builder">Edit with '. apply_filters('betheme_label', "Muffin") .' Live Builder</a></span>';
    }
    return $actions;
}
